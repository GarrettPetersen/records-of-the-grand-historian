...
function kindleCoverImgHtml({ className = 'kindle-promo-cover-img', width = 108 }) {
  const { coverPath, title, coverColor } = SHIJI_KINDLE;
  const height = Math.round(width * (2560 / 1600));
  return `<div class="${className}" style="--book-color: ${coverColor}"><img src="${coverPath}" alt="${title} cover" width="${width}" height="${height}" loading="lazy" decoding="async" /></div>`;
}
...
export function kindleFooterLine(prefix = '') {
  const { amazonUrl, chinese, disclosure } = SHIJI_KINDLE;
  const rel = amazonLinkRel();
  return `<p class="kindle-footer-link"><a href="${amazonUrl}" target="_blank" rel="${rel}">${chinese} — Kindle edition</a> · <span class="affiliate-disclosure">${disclosure}</span> · <a href="${prefix}privacy.html#amazon-associates">Details</a></p>`;
}
...
export function kindleInlineCalloutHtml({ variant = 'hub' } = {}) {
  ...
  const body = variant === 'chapter'
    ? `The complete English translation of the <em>${title}</em> (${chinese}) is available as a Kindle e-book for uninterrupted reading.`
    : `The complete English translation of the <em>${title}</em> (${chinese}) is now available as a Kindle e-book — polished for long-form reading away from the browser.`;

  const coverWidth = variant === 'hub' ? 108 : 84;
...
export function kindleModalMarkup() {
  ...
  <div class="kindle-promo-panel" role="document">
    ...
    ${kindleCoverImgHtml({ className: 'kindle-promo-cover', width: 120 })}
    <p class="kindle-promo-eyebrow">New</p>
    <h2 id="kindle-promo-title" class="kindle-promo-title">${chinese} on Kindle</h2>
