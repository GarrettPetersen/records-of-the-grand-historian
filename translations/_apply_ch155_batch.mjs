#!/usr/bin/env node
/** Apply translations/_ch155_bN.mjs to current_translation_zizhitongjian.json by sentence index. */
import fs from 'node:fs';
import path from 'node:path';
import { pathToFileURL } from 'node:url';

const batchNum = Number(process.argv[2]);
if (!batchNum || batchNum < 2) {
  console.error('Usage: node translations/_apply_ch155_batch.mjs <batchNumber>=2..5');
  process.exit(1);
}

const transPath = path.resolve('translations/current_translation_zizhitongjian.json');
const batchPath = path.resolve(`translations/_ch155_b${batchNum}.mjs`);
