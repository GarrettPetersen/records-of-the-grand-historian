...
- **Local parallel SDK (`sdk-translate:local` with concurrency >1):** always use `make continue BOOK=<book>` (per-book lock file `translations/.continue_book.<book>.tmp`). Per book, session *N+1* starts only after the previous chapter is on `origin/master` (merge-wait); books do not block each other. After changing orchestrator code, drain and restart (`npm run sdk-translate:drain`) — a running process does not reload the script.
