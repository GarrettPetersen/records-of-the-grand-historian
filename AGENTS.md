...
- **SDK translation orchestrator:** Per book, session *N+1* starts only after the previous chapter is on `origin/master` (merge-wait); books do not block each other (`--concurrency`). Local: `npm run sdk-translate:local -- --all-untranslated --concurrency 4 --until-complete` (direct push). Cloud: `npm run sdk-translate:cloud -- --all-untranslated --concurrency 19 --until-complete` (PR + automerge). `make continue` uses per-book lock files `translations/.continue_book.<book>.tmp` (matters for local parallel). Drain before restart: `npm run sdk-translate:drain`.
