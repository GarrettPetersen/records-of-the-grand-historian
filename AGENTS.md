...
- **Translation PR batching (Cloudflare builds):** Workflow `automerge-cursor.yml` only labels `cursor/*` PRs (`translation-batch`); it does **not** auto-merge to `master`. Absorb open chapter PRs into `translation-staging` with `npm run merge-translation:staging` or GitHub Actions **Merge translation PRs to staging**, then **one** squash PR `translation-staging` → `master` (one ~10m build). `npm run merge-translation:staging:dry-run` lists queued PRs.
