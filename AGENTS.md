...
- **Build/rebuild site:** `make update BOOK=<book>` — runs citations, counts, manifest (merge), progress (merge), static HTML, **Open Graph PNGs** (`generate-og-images.js` with **`--incremental` by default**: skips a PNG when it is at least as new as its source JSON/manifest), and sync for that book only. Use `make update-all` for every book under `data/`. To **re-raster every OG card** after layout/font changes, run with **`OG_FULL=1`** (e.g. `make update-all OG_FULL=1`).
