...
- **Translation PR batching (Cloudflare builds):** Workflow `automerge-cursor.yml` only labels `cursor/*` PRs (`translation-batch`); it does **not** auto-merge to `master`. `npm run merge-translation:staging` (or workflow **Merge translation PRs to staging**) retargets each open chapter PR to `translation-staging` and **merges it via GitHub** so chapter PRs show as merged; then **one** squash PR `translation-staging` → `master` (one ~10m build). `npm run merge-translation:staging:dry-run` lists queued PRs. `--git-only` keeps the old absorb-via-local-git behavior without closing PRs on GitHub.
