...
- **Cloudflare Pages:** In the project **Settings → Builds**, set **Build command** to **`npm run build`** (cannot be empty). **Build output directory:** **`public`**. Git-connected Pages does **not** read a build command from `wrangler.toml`—only the dashboard (or API) sets that. An empty build command skips the whole pipeline and deploys committed `public/` as-is (no OG PNG generation, etc.). Regenerates manifest, progress, static HTML, and **all** `public/og/**/*.png` files. **Do not gitignore those PNGs:** Pages can omit gitignored build outputs from the deploy. The build ends with `scripts/verify-og-assets.mjs` to require a valid `public/og/site.png`.
