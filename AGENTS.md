...
- **Local parallel SDK (`sdk-translate:local` with concurrency >1):** always use `make continue BOOK=<book>` (per-book lock file `translations/.continue_book.<book>.tmp`). To stop without killing agents: `npm run sdk-translate:drain`, wait for `drain complete` in the log, then restart.
